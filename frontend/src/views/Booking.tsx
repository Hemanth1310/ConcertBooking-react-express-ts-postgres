...
const Booking = () => {
  ...
  const handleBooking = async () => {
    try {
      const bookingData = await api.post(
        `/api/booking/${id}/${ticketType}?qty=${quantity}`
      );
      ...
      <div className="flex flex-col md:flex-row gap-10">
        <div className="flex-2 flex flex-col gap-5">
          <div className="flex-1">
            <img
              className="h-64 w-auto md:h-128"
            ...
            {TicketInfoById ? (
              <div className="flex flex-col gap-3">
                ...
                <div className="flex gap-10 text-3xl items-center w-full justify-center px-10">
                  <button
                    disabled={quantity < 1}
                    className="font-bold hover:text-white hover:bg-black p-3 rounded-2xl cursor-pointer disabled:cursor-not-allowed disabled:text-gray-400"
                    onClick={() => setQuantity((prev) => prev - 1)}
                  >
                    -
                  </button>
                  <div className="border-2 border-black  text-center w-16 rounded-2xl">
                    {quantity}
                  </div>
                  <button
                    disabled={quantity >= TicketInfoById![0].availableQuantity}
                    className="font-bold hover:text-white hover:bg-black p-3 rounded-2xl cursor-pointer disabled:cursor-not-allowed disabled:text-gray-400"
                    onClick={() => setQuantity((prev) => prev + 1)}
                  >
                    +
                ...
                <div>
                  {quantity >= TicketInfoById![0].availableQuantity && (
                    <div>You are trying to book all the tickets</div>
                  )}
                </div>
                <div className="flex gap-10 text-2xl items-center">
                  <div className="">
